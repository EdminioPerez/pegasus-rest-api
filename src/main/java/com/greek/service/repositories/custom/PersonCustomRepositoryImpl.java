...
public class PersonCustomRepositoryImpl implements PersonCustomRepository {
    ...
    public Page<Persona> findPersonsLike(
            @Param("globalFilter") String globalFilter, Pageable pageable) {
        ...
        if (StringUtils.isNotBlank(globalFilter)) {
            String[] busqueda =
                    StringUtils.splitByWholeSeparator(
                            Normalizer.normalize(globalFilter, Normalizer.Form.NFKD)
            ...
            for (int x = 0; x < busqueda.length; ++x) {
                ...
                query.append(
                        "(upper(p.nombrePersona) like upper(?"
                                + paramPlace
                                + ") or upper(p.apellidoPersona) like upper(?"
                                + paramPlace
                                + "))");
                queryCount.append(
                        "(upper(p.nombrePersona) like upper(?"
